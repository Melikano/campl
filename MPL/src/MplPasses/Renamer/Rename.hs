...
import Text.Show.Pretty qualified as PrettyShow
...
renameDefn (TypeClassDefn (SeqTypeClass (MplSeqTypeClass name t tvars funs supers))) = do
  name' <- tagIdentP name
  tvars' <-
    mapM
      ( \var -> do
          (_, tvar) <- renameType var
          return tvar
      )
      tvars
  t' <- renameType t
  supers' <-
    mapM
      ( \(sname, svars, stype) -> do
          sname' <- tagIdentP sname
          svars' <- mapM renameTVar svars
          stype' <- renameType stype
          return (sname', svars', stype')
      )
      supers
  funs' <-
    mapM
      renameTypeClassFuns
      funs
  return $ TypeClassDefn $ SeqTypeClass $ _MplSeqTypeClass # (name', t', tvars', funs', supers')
renameDefn (TypeClassDefn (ConcTypeClass (MplConcTypeClass name t tvars procs seqSupers concSupers))) = do
  name' <- tagIdentP name
  tvars' <- mapM renameTVar tvars
  t' <- renameType t
  seqSupers' <- mapM renameSupers seqSupers
...
renameTVar :: Rename (MplType MplParsed) (MplType MplRenamed)
renameTVar var = do
  (_, tvar) <- renameType var
  return tvar
...
renameTypeClassFuns :: Rename (TypeClassFun MplParsed) (TypeClassFun MplRenamed)
renameTypeClassFuns (TypeClassFun funName funType funDefn) = do
  name' <- tagIdentP funName
  ...
  defn' <- traverse renamePattsExpr funDefn
  return $ TypeClassFun name' funType' defn'

renameTypeClassProcs :: Rename (TypeClassProc MplParsed) (TypeClassProc MplRenamed)
renameTypeClassProcs (TypeClassProc procName procType procDefn) = do
  procName' <- tagIdentP procName
  proctype' <- case procType of
    Just (seqs, ins, outs) -> do
      lcl <- guse envLcl
      (bds0, seqs') <- unzip <$> traverse renameType seqs
      (bds1, ins') <- unzip <$> traverse renameType ins
      (bds2, outs') <- unzip <$> traverse renameType outs
      ...
      return $ Just (fold (bds0 <> bds1 <> bds2), seqs', ins', outs')
    Nothing -> return Nothing
  defn' <- traverse renameProcBodyPhrase procDefn

  return $ TypeClassProc procName' proctype' defn'
