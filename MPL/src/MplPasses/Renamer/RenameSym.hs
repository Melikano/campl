...
lookupSym ident prism ((ident', entry) : rst)
  ...
  | ident == ident',
    Just entry' <- entry ^? symEntryInfo % prism =
  ...
  collectSymTab (ObjectDefn n) = case n of
    ...
    where
      querySpines spine tp tpphrase =
        foldMapOf
          (typeClauseSpineClauses % folded)
          (flip queryTypeClause tp)
          spine
          ++ foldMapOf
            (typeClauseSpineClauses % folded)
            (flip queryTypePhrase tpphrase)
            spine

      queryTypeClause clause tp =
      ...
      queryTypePhrase clause tpphrase =
        foldMapOf
          (typeClausePhrases % traversed % typePhraseName)
          ( set
              (mapped % _2 % symEntryInfo)
              (_Just # tpphrase)
              . collectSymTab
...
instance CollectSymTab ChIdentR where
  collectSymTab ch =
    (ch ^. chIdentRIdentR % to collectSymTab)
      & mapped
      % _2
      % symEntryInfo
      .~ _Just
