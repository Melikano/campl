...
parseBnfcTypeClass :: BnfcParse B.TypeClassDefn (MplTypeClass MplParsed)
parseBnfcTypeClass (B.TYPECLASS_DEFN constraint methods) = do
...
parseSuperClasses (B.SUPER_CLASSES_SEQ classes) = do
  ...
  return (parsedSuperclasses, [])
parseSuperClasses (B.SUPER_CLASSES_CONC seqClasses concClasses) = do
  parsedSeqSupers <- mapM parseTypeClassConstraint seqClasses
...
parseTypeClassFun :: BnfcParse B.ClassProp (TypeClassFun MplParsed)
parseTypeClassFun (B.FUN_CLASS_PROP fun) =
  case fun of
    B.FUNCTION_SIGN id froms to -> do
      froms' <- traverseTryEach parseBnfcType froms
      to' <- parseBnfcType to
      return $ TypeClassFun (toTermIdentP id) (Just (froms', to')) []
    B.FUNCTION_IMPL id froms to pattsexprs -> do
      froms' <- traverseTryEach parseBnfcType froms
      to' <- parseBnfcType to
      pattsexprs' <- traverseTryEach parseBnfcPattsExpr pattsexprs
      return $ TypeClassFun (toTermIdentP id) (Just (froms', to')) pattsexprs'
parseTypeClassFun _ = error "Parse Error: Can't define proc inside a sequential type class"

parseTypeClassProc :: BnfcParse B.ClassProp (TypeClassProc MplParsed)
parseTypeClassProc (B.PROC_CLASS_PROP proc) =
  case proc of
    B.PROCESS_SIGN id seqtype intype outtype -> do
      ~[seqtype', intype', outtype'] <- traverseTryEach (traverseTryEach parseBnfcType) [seqtype, intype, outtype]
      return $ TypeClassProc (toChIdentP id) (Just (seqtype', intype', outtype')) []
    B.PROCESS_IMPL id seqtype intype outtype phrases -> do
      ~[seqtype', intype', outtype'] <- traverseTryEach (traverseTryEach parseBnfcType) [seqtype, intype, outtype]
      phrases' <- traverseTryEach parseBnfcProcessPhrase phrases
