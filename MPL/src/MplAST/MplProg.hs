...
type family XSeqTypeClassDefn x

type family XConcTypeClassDefn x

type family XSeqTypeClassInstanceDefn x

type family XConcTypeClassInstanceDefn x

type ForallDefn (c :: Type -> Constraint) x =
  ( ForallObjDefn c x,
    c (XFunctionDefn x),
    c (XProcessDefn x),
    c (XSeqTypeClassDefn x),
    c (XConcTypeClassDefn x),
    c (XSeqTypeClassInstanceDefn x),
    c (XConcTypeClassInstanceDefn x),
    c (MplTypeClass x),
    c (MplTypeClassInstance x)
  )
...
data MplDefn x
  ...
  | TypeClassDefn !(MplTypeClass x)
  | TypeClassInstanceDefn !(MplTypeClassInstance x)

-- data type for type class
data MplTypeClass x
  = SeqTypeClass !(XSeqTypeClassDefn x)
  | ConcTypeClass !(XConcTypeClassDefn x)
...
deriving instance
  (Show (XSeqTypeClassInstanceDefn x), Show (XConcTypeClassInstanceDefn x)) =>
  Show (MplTypeClassInstance x)
...
-- data types for sequential type classes
data TypeClassFun x = TypeClassFun
  { _typeClassFunName :: IdP x,
    _typeClassFunType :: XFunType x,
    _typeClassFunDefn :: [([XMplPattern x], XMplExpr x)]
  }
...
type TypeClassConstraint x = [(IdP x, [IdP x], MplType x)]

data MplSeqTypeClass x = MplSeqTypeClass
  { _seqTypeClassName :: IdP x,
    _seqTypeClassConstrainedType :: MplType x,
...
deriving instance
  (Show (IdP x), Show (TypeClassFun x), Show (MplType x)) =>
  Show (MplSeqTypeClass x)

$(makeLenses ''MplSeqTypeClass)

-- data types for concurrent type classes
data TypeClassProc x = TypeClassProc
  { _typeClassProcName :: IdP x,
    _typeClassProcType :: XProcType x,
    _typeClassProcDefn :: [(([XMplPattern x], [ChP x], [ChP x]), NonEmpty (XMplCmd x))]
  }

deriving instance
  (Show (IdP x), Show (XProcType x), Show (XMplPattern x), Show (XMplCmd x), Show (ChP x)) =>
  Show (TypeClassProc x)

data MplConcTypeClass x = MplConcTypeClass
  { _concTypeClassName :: IdP x,
    _concTypeClassConstrainedType :: MplType x,
...
deriving instance
  (Show (IdP x), Show (MplProcess x), Show (MplType x)) =>
