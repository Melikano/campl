...
type instance
  XTypePhraseFrom MplRenamed (SeqObjTag DataDefnTag) =
    [XMplType MplRenamed]

type instance
  XTypePhraseTo MplRenamed (SeqObjTag DataDefnTag) =
    TypeP MplRenamed

type instance
  XTypePhraseFrom MplRenamed (SeqObjTag CodataDefnTag) =
    ([XMplType MplRenamed], TypeP MplRenamed) -- args ++ [statevar] State var must be the last variable

type instance
  XTypePhraseTo MplRenamed (SeqObjTag CodataDefnTag) =
    XMplType MplRenamed

type instance
  XTypePhraseFrom MplRenamed (ConcObjTag ProtocolDefnTag) =
    XMplType MplRenamed

type instance
  XTypePhraseTo MplRenamed (ConcObjTag ProtocolDefnTag) =
    TypeP MplRenamed

type instance
  XTypePhraseFrom MplRenamed (ConcObjTag CoprotocolDefnTag) =
    TypeP MplRenamed

type instance
  XTypePhraseTo MplRenamed (ConcObjTag CoprotocolDefnTag) =
    XMplType MplRenamed
...
type instance
  XProcType MplRenamed =
    Maybe ([IdentR], [XMplType MplRenamed], [XMplType MplRenamed], [XMplType MplRenamed])

type instance XTypeClassType MplRenamed = ([IdentR], XMplType MplRenamed)
