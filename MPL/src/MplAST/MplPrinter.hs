...
import MplLanguage.AbsMPL qualified as B
import MplLanguage.ErrM qualified as B
import MplLanguage.LayoutMPL qualified as B
import MplLanguage.LexMPL qualified as B
import MplLanguage.ParMPL qualified as B
import MplLanguage.PrintMPL qualified as B
...
instance UserProvidedTypeToBnfc ([ident], MplType MplRenamed) x where
  userProvidedTypeToBnfc proxy (foralls, t) = Just $ mplTypeToBnfc proxy t
...
type MplPrintConstraints x y =
  ( XDataDefn x ~ MplTypeClauseSpine x (SeqObjTag DataDefnTag),
    ...
    XSeqTypeClassDefn x ~ MplSeqTypeClass x,
    XConcTypeClassDefn x ~ MplConcTypeClass x,
    XSeqTypeClassInstanceDefn x ~ MplSeqTypeClassInstance x,
    XConcTypeClassInstanceDefn x ~ MplConcTypeClassInstance x,
    ...
    UserProvidedTypeToBnfc (XTypeClassType x) y,
    (PPrint (TypeP x) y)
  )
  ...
  where
    ...
    f ((patts, ins, outs), cmds) =
      B.PROCESS_PHRASE
        ...
        (map (toBnfcIdent proxy) outs)
        $ mplCmdsToBnfc proxy cmds
mplDefnToBnfc proxy (TypeClassDefn (SeqTypeClass (MplSeqTypeClass name t tvars funs supers))) = B.MPL_TYPECLASS_DEFN $ if null supers then B.TYPECLASS_DEFN constraint funs' else B.TYPECLASS_SUPERCLASS_DEFN supers' constraint funs'
  where
    funs' = mplTypeClassFunToBnfc proxy <$> funs
    supers' = B.SUPER_CLASSES_SEQ $ mplTypeClassConstraintToBnfc proxy <$> supers
    constraint = mplTypeClassConstraintToBnfc proxy (name, tvars, t)
mplDefnToBnfc proxy (TypeClassDefn (ConcTypeClass (MplConcTypeClass name t tvars procs seqSupers concSupers))) =
  B.MPL_TYPECLASS_DEFN $
    if null seqSupers && null concSupers
      then B.TYPECLASS_DEFN constraint procs'
      else B.TYPECLASS_SUPERCLASS_DEFN supers' constraint procs'
  where
    procs' = mplTypeClassProcToBnfc proxy <$> procs
    supers' = B.SUPER_CLASSES_CONC (mplTypeClassConstraintToBnfc proxy <$> seqSupers) (mplTypeClassConstraintToBnfc proxy <$> concSupers)
    constraint = mplTypeClassConstraintToBnfc proxy (name, tvars, t)
mplDefnToBnfc proxy (TypeClassInstanceDefn (SeqTypeClassInstance (MplSeqTypeClassInstance t tvars funs deps))) =
  B.MPL_INSTANCE_DEFN $
    if null deps
      then B.TYPECLASS_INSTANCE_DEFN constraint funs'
      else B.TYPECLASS_INSTANCE_DEPENDENCY_DEFN deps' constraint funs'
  where
    funs' = mplTypeClassFunDefnToBnfc proxy <$> funs
    deps' = B.SUPER_CLASSES_SEQ (mplTypeClassConstraintToBnfc proxy <$> deps)
    constraint = mplTypeClassConstraintToBnfc proxy ("", tvars, t)
mplDefnToBnfc proxy (TypeClassInstanceDefn (ConcTypeClassInstance (MplConcTypeClassInstance t tvars procs seqDeps concDeps))) =
  B.MPL_INSTANCE_DEFN $
    if null seqDeps && null concDeps
      then B.TYPECLASS_INSTANCE_DEFN constraint procs'
      else B.TYPECLASS_INSTANCE_DEPENDENCY_DEFN deps' constraint procs'
  where
    procs' = mplTypeClassProcDefnToBnfc proxy <$> procs
    deps' = B.SUPER_CLASSES_CONC (mplTypeClassConstraintToBnfc proxy <$> seqDeps) (mplTypeClassConstraintToBnfc proxy <$> concDeps)
    constraint = mplTypeClassConstraintToBnfc proxy ("", tvars, t)
mplDefnToBnfc _ _ = error "bad 'mplDefnToBnfc' use"

mplTypeClassConstraintToBnfc proxy (id, tvars, t) =
  if null tvars then B.TYPE_CONSTRAINT id' t' else B.TYPE_CONSTRAINT_HIGHER_ORDER id' tvars' t'
  where
    id' = toBnfcIdent proxy id
    tvars' =
      ( \t -> case mplTypeToBnfc proxy t of
          B.MPL_UIDENT_NO_ARGS_TYPE x -> x
          _ -> error "erro"
      )
        <$> tvars
    t' = case userProvidedTypeToBnfc proxy t of
      Nothing -> error "error: unknown type"
      Just tp -> tp

mplTypeClassFunToBnfc ::
  MplPrintConstraints x y =>
  Proxy y ->
  TypeClassFun x ->
  B.ClassProp
mplTypeClassFunToBnfc proxy (TypeClassFun id ftype fdef) =
  B.FUN_CLASS_PROP $
    if null fdef
      then B.INTERNAL_FUNCTION_SIGN id' t'
      else B.INTERNAL_FUNCTION_IMPL id' t' fdef'
  where
    t' =
      case userProvidedTypeToBnfc proxy ftype of
        Nothing -> error "error: unkown type of class method"
        Just tp -> tp
    id' = toBnfcIdent proxy id
    fdef' =
      fmap
...
mplTypeClassProcToBnfc ::
  MplPrintConstraints x y =>
  Proxy y ->
  TypeClassProc x ->
  B.ClassProp
mplTypeClassProcToBnfc proxy (TypeClassProc id ptype pdef) =
  B.PROC_CLASS_PROP $
    if null pdef
      then B.INTERNAL_PROCESS_SIGN id' t'
      else B.INTERNAL_PROCESS_IMPL id' t' pdef'
  where
    t' =
      case userProvidedTypeToBnfc proxy ptype of
        Nothing -> error "error: unkown type of class method"
        Just tp -> tp
    id' = toBnfcIdent proxy id
    pdef' = fmap f pdef
...
mplTypeClassProcDefnToBnfc ::
  MplPrintConstraints x y =>
  Proxy y ->
  MplProcess x ->
  B.ClassPropDefn
mplTypeClassProcDefnToBnfc proxy p =
  B.PROCESS_DEF p'
  where
    p' = case mplDefnToBnfc proxy (ProcessDefn p) of
      B.MPL_PROCESS_DEFN x -> x
      _ -> error "error transforming process method to bnfc"

mplTypeClassFunDefnToBnfc ::
  MplPrintConstraints x y =>
  Proxy y ->
  MplFunction x ->
  B.ClassPropDefn
mplTypeClassFunDefnToBnfc proxy f =
  B.FUNCTION_DEF f'
  where
    f' = case mplDefnToBnfc proxy (FunctionDefn f) of
      B.MPL_FUNCTION_DEFN x -> x
      _ -> error "error transforming fun method to bnfc"
