...
type instance XSeqTypeClassDefn MplLambdaLifted = MplSeqTypeClass MplLambdaLifted
...
type instance
  XEUnfoldPhrase MplLambdaLifted =
    ()

type instance
  XEUnfoldSubPhrase MplLambdaLifted =
...
type instance
  XTypePhraseTo MplLambdaLifted t =
    XMplType MplTypeChecked

type instance
  XTypePhraseFrom MplLambdaLifted (SeqObjTag DataDefnTag) =
    [XMplType MplTypeChecked]

type instance
  XTypePhraseFrom MplLambdaLifted (SeqObjTag CodataDefnTag) =
    ([XMplType MplTypeChecked], XMplType MplTypeChecked) -- args ++ [statevar] State var must be the last variable

type instance
  XTypePhraseFrom MplLambdaLifted (ConcObjTag ProtocolDefnTag) =
    XMplType MplTypeChecked

type instance
  XTypePhraseFrom MplLambdaLifted (ConcObjTag CoprotocolDefnTag) =
    XMplType MplTypeChecked

-- Function / process type
type instance
  XFunType MplLambdaLifted =
    ([TypeP MplTypeChecked], [XMplType MplTypeChecked], XMplType MplTypeChecked)

type instance
  XProcType MplLambdaLifted =
...
type instance
  XTypeConcWithArgs MplLambdaLifted =
...
type instance
  XTypeVar MplLambdaLifted =
    Maybe (XMplKind MplLambdaLifted)

type instance
  XTypeWithNoArgs MplLambdaLifted =
